...
program
  .name('Nexus Bridge CLI')
  .description('A command line interface (CLI) for managing localization keys and values for i18nexus')
  .version(pkg.version);
...
program.command('pull-latest').alias('p')
  ...
  .option('-s, --namespaces [namespaces...]', 'Name/names of namespaces to retrieve if you do not want all of them', [])
...
program.command('import').alias('i')
  ...
  .requiredOption('-s, --namespace <name>', 'Namespace in i18nexus')
...
program.command('update-string').alias('u')
  .description('Update a single base string key, value or details')
  .requiredOption('-k, --api-key <apiKey>', 'The API key for your project', process.env.I18NEXUS_API_KEY)
  .requiredOption('-t, --token <token>', 'A personal access token generated for your account in i18nexus.', process.env.I18NEXUS_API_BEARER)
  .requiredOption('-s, --namespace <namespace>', 'Namespace in i18nexus')
